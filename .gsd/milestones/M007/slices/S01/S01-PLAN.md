...
- [x] **T01: Collapse AnomalyTable by default and add component test** `est:20m`
...
- [x] **T02: Create SanctionedVessels component and wire into FleetPage** `est:30m`
...
- **Runtime signals:** `aria-expanded` on each `AnomalyTable` toggle button reflects collapsed/expanded state inspectable via DevTools accessibility tree. `SanctionedVessels` section renders with `data-testid="sanctioned-vessels"` when present.
- **Inspection surfaces:** The `/fleet` page renders anomaly group counts in header brackets (e.g. `[3]`) visible without expanding. Sanctioned vessel count visible in section header.
- **Failure visibility:** If no anomalies load, the fleet page renders an empty state — no anomaly tables or sanctions section appear. Component tests validate both collapsed default state and empty-array behavior.
- **Diagnostic verification:** `npx vitest run src/components/fleet/__tests__/AnomalyTable.test.tsx` — validates toggle state transitions render correctly under happy-dom. If `aria-expanded` assertions fail, the toggle wiring is broken.
